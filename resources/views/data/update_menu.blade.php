...
<div class="custom-body m-4">
  ...
  <form action="/update_menu/<?php echo $data['data']['id'] ?>" method="post" class="post-form p-4 d-i-block" enctype="multipart/form-data">
    {{ csrf_field() }}

    <h3 class="mt-0 mb-3 txt-normal">Form Tambah Menu</h3>
    <label for="" class="default-fade">Nama Menu</label>
    <input class="input mb-3" type="text" name="nama_menu" value="<?php echo $data['data']['nama_menu'] ?>">
    <label for="" class="default-fade">Harga</label>
    <input class="input mb-3" type="number" name="harga" value="<?php echo $data['data']['harga'] ?>">
    ...
    <br>
    <button type="submit" class="btn mt-2" name="update_menu">Submit</button>
